...
{
	unsigned int f_log = open("logs/actions.log", O_WRONLY | O_APPEND | O_CREAT, S_IRWXU | S_IRWXG); // S_IRWXU - user has read, write and execute permission, S_IRWXG - group has read, write and execute permission
	time_t t = time(NULL);
